...
class Target(BaseModel):
    ...
    resource_tags: Optional[Dict[str, str]] = Field(None, description="Dictionary of resource tags to target")
    filters: Optional[List[Dict[str, Any]]] = Field(None, description="List of filters to apply to the target")
...
class Action(BaseModel):
    ...
    targets: Optional[Dict[str, str]] = Field(None, description="Dictionary of targets for the action")
    start_after: Optional[List[str]] = Field(None, description="List of action IDs that must complete before this action starts")
...
class LogConfiguration(BaseModel):
    ...
    cloud_watch_logs_configuration: Optional[Dict[str, Any]] = Field(None, description="CloudWatch logs configuration")
...
class ExperimentTemplateRequest(BaseModel):
    ...
    stop_conditions: List[StopCondition] = Field(..., description="Conditions that stop the experiment")
    ...
    log_configuration: Optional[LogConfiguration] = Field(None, description="Configuration for experiment logging")
...
class StartExperimentRequest(BaseModel):
    ...
    action: ExperimentActionsMode = Field(ExperimentActionsMode.RUN_ALL, description="The actions mode")
    max_timeout_seconds: int = Field(3600, description="Maximum time to wait for experiment completion")
